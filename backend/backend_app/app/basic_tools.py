...
@lru_cache(maxsize=1)
def _get_tavily():
    """Return a Tavily SearchResults tool using a cached API wrapper."""
...
def _get_tavily_answer():
    """Return a Tavily short-answer tool reusing the same cached API wrapper."""
    # Reuse the same underlying Tavily API wrapper instance to avoid
    # duplicate initialization and extra network/config overhead.
    tavily_search = TavilySearchAPIWrapper()
...
def _get_connery_actions():
    """Return Connery toolkit actions, but fail-safe to an empty list.

    Connery is an optional integration; if loading the toolkit fails we
    return an empty list to avoid crashing the application at import time.
    """
    try:
        ...
        # Log could be added here; return empty list so callers can proceed.
        return []
...
def resolve_tool(tool_enum: AvailableTools):
    """Helper to resolve a tool factory from TOOLS and call it.

    Returns the tool (or list of tools) produced by the factory, or None
    if the tool is not available. This centralizes error handling when
    optional dependencies are missing.
    """
    factory = TOOLS.get(tool_enum)
    if not factory:
        return None
    try:
        return factory()
